...
async function main(): Promise<void> {
  ...
  const capCall = deed.interface.encodeFunctionData("configureCityConcentrationCap", [
    TEST_CITY_ID,
    10,
  ]);
  await (
    await timelock
      .connect(admin)
      .schedule(deedAddress, 0, capCall, ZERO_BYTES32, ZERO_BYTES32, TIMELOCK_DELAY)
  ).wait();
  await time.increase(TIMELOCK_DELAY + 1);
  await (
    await timelock.connect(admin).execute(deedAddress, 0, capCall, ZERO_BYTES32, ZERO_BYTES32)
  ).wait();
  const [cap, configured] = await deedClaims.cityConcentrationCap(TEST_CITY_ID);
  line(`city ${TEST_CITY_ID} per-holder concentration cap ${cap}, configured ${configured}`);
